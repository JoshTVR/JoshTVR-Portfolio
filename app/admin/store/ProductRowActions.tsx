...
export function ToggleActiveButton({ id, active }: { id: string; active: boolean }) {
  ...
  return (
    <button
      onClick={() => startTransition(async () => { await toggleActive(id, !active); window.location.reload() })}
      disabled={isPending}
...
export function DeleteProductButton({ id, name }: { id: string; name: string }) {
  ...
  return (
    <button
      onClick={() => { if (!confirm(`Delete "${name}"?`)) return; startTransition(async () => { await deleteProduct(id); window.location.reload() }) }}
      disabled={isPending}
