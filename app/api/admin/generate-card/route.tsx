...
export async function GET(req: NextRequest) {
  ...
  // Auth: same CRON_SECRET used by the cron
  ...
  if (post.card_type === 'code_tip') {
    if (slide === 0) {
      ...
      return new ImageResponse(
        <div style={{
          ...
          <div style={{
            marginTop: 40, fontSize: 48, fontWeight: 800, color: theme.text,
            lineHeight: 1.2, maxWidth: 900,
          }}>
            {str('question') || post.title_en}
          </div>
          <div style={{
            marginTop: 24, fontSize: 22, color: theme.textMuted, lineHeight: 1.5, maxWidth: 860,
          }}>
    ...
    if (slide === 1) {
      ...
      return new ImageResponse(
        <div style={{
          width: '100%', height: '100%', display: 'flex', flexDirection: 'column',
          background: theme.bg, padding: 48, overflow: 'hidden',
        }}>
          <div style={{
            ...
            <div style={{
              ...
              background: `rgba(${theme.accentRgb},0.15)`,
              color: theme.accent, fontSize: 13, fontWeight: 700, display: 'flex',
            }}>
              {lang.toUpperCase()}
            </div>
          </div>
    ...
    return new ImageResponse(
      <div style={{
        width: '100%', height: '100%', display: 'flex', flexDirection: 'column',
        background: theme.bg, padding: 64, overflow: 'hidden',
      }}>
        <Badge text="💡 Takeaway" color={theme.accent} />
        <div style={{
          marginTop: 32, fontSize: 30, color: theme.text, lineHeight: 1.6,
          maxWidth: 920,
        }}>
          {str('explanation')}
        </div>
  ...
  if (post.card_type === 'qa') {
      ...
      return new ImageResponse(
        <div style={{
          ...
          <div style={{
            ...
            fontSize: 34, fontWeight: 700, color: theme.text, lineHeight: 1.45,
            display: 'flex',
          }}>
            {str('question')}
          </div>
          <div style={{ marginTop: 32, fontSize: 22, color: theme.textMuted }}>
            👇 Swipe para ver la respuesta
          </div>
          <Brand />
        </div>,
        { width: 1080, height: 1080 },
    ...
    return new ImageResponse(
      <div style={{
        ...
        <div style={{
          marginTop: 28, fontSize: 26, color: theme.text, lineHeight: 1.6, maxWidth: 940,
        }}>
          {str('answer').slice(0, 300)}
        </div>
        {hasCode && (
          <div style={{
  ...
  if (post.card_type === 'logic_challenge') {
    ...
    if (slide === 0) {
      return new ImageResponse(
        <div style={{
          ...
          <div style={{
            fontSize: 28, color: theme.text, lineHeight: 1.6, maxWidth: 940,
          }}>
            {str('challenge')}
          </div>
          <div style={{ marginTop: 32, fontSize: 20, color: theme.textMuted }}>
            ⏱️ ¿Puedes resolverlo? Swipe para la solución →
          </div>
          <Brand />
        </div>,
        { width: 1080, height: 1080 },
    ...
    return new ImageResponse(
      <div style={{
        ...
        <div style={{
          marginTop: 28, fontSize: 26, color: theme.text, lineHeight: 1.7,
          maxWidth: 940, whiteSpace: 'pre-wrap',
        }}>
          {str('solution').slice(0, 500)}
        </div>
        <Brand />
      </div>,
      { width: 1080, height: 1080 },
  ...
  if (post.card_type === 'study') {
    return new ImageResponse(
      <div style={{
        ...
        <div style={{
          fontSize: 100, color: `rgba(${theme.accentRgb},0.15)`, lineHeight: 0.8,
          fontFamily: 'Georgia, serif', display: 'flex',
        }}>
          "
        </div>
        <div style={{
          fontSize: 30, fontWeight: 700, color: theme.text, lineHeight: 1.45,
          maxWidth: 920, marginTop: 8,
        }}>
          {str('finding').slice(0, 300)}
        </div>
        {!!data.source && (
          <div style={{
  ...
  if (post.card_type === 'announcement') {
    return new ImageResponse(
      <div style={{
        width: '100%', height: '100%', display: 'flex', flexDirection: 'column',
        background: theme.bg, padding: 64, overflow: 'hidden', position: 'relative',
      }}>
        <div style={{
          position: 'absolute', top: -80, right: -80, width: 360, height: 360,
          borderRadius: 180, display: 'flex',
          background: `radial-gradient(circle, rgba(${theme.accentRgb},0.2), transparent 70%)`,
        }} />
        <Badge text="📢 Announcement" color={theme.accent} />
        <div style={{
          marginTop: 32, fontSize: 52, fontWeight: 800, color: theme.text, lineHeight: 1.1,
        }}>
          {str('headline') || post.title_en}
        </div>
        <div style={{
          marginTop: 28, fontSize: 26, color: theme.textMuted, lineHeight: 1.55, maxWidth: 880,
        }}>
          {str('description').slice(0, 200)}
        </div>
        {!!data.cta && (
          <div style={{
            ...
            color: theme.accent, fontSize: 20, fontWeight: 700, width: 'fit-content',
          }}>
            {str('cta')} →
          </div>
        )}
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', gap: 10, alignItems: 'center', marginBottom: 24 }}>
          ...
          {!!data.milestone && (
            <div style={{
              padding: '6px 16px', borderRadius: 100, display: 'flex',
              background: 'rgba(251,191,36,0.12)', color: '#fbbf24', fontSize: 13, fontWeight: 700,
            }}>
              {str('milestone')}
            </div>
          )}
        ...
        <div style={{
          fontSize: 30, color: theme.text, lineHeight: 1.65, maxWidth: 940,
        }}>
          {str('update').slice(0, 400)}
        </div>
        {Array.isArray(data.tech_tags) && (
          <div style={{ display: 'flex', gap: 8, marginTop: 28, flexWrap: 'wrap' }}>
            {(data.tech_tags as string[]).slice(0, 5).map((t, i) => (
              <div key={i} style={{
                padding: '6px 14px', borderRadius: 8, display: 'flex',
                background: `rgba(${theme.accentRgb},0.08)`,
  ...
  return new ImageResponse(
    <div style={{
      ...
      <div style={{ fontSize: 20, color: theme.textMuted, marginBottom: 32 }}>
        joshtvr.com
      </div>
      <div style={{
        fontSize: 52, fontWeight: 800, color: theme.text, lineHeight: 1.1, maxWidth: 900,
      }}>
        {post.title_en}
      </div>
      <Brand />
