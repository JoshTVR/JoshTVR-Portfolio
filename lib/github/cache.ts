...
import { fetchGitHubStats, fetchPublicRepos, type GitHubStats, type GitHubRepo } from './api'

const CACHE_KEY       = 'github_stats'
const REPOS_CACHE_KEY = 'github_repos'
...
export async function getPublicRepos(): Promise<GitHubRepo[]> {
  ...
  try {
    ...
    const { data: cached } = await supabase
      .from('github_cache')
      .select('data, updated_at')
      .eq('key', REPOS_CACHE_KEY)
      .single()

    if (cached) {
      const age = Date.now() - new Date(cached.updated_at).getTime()
      if (age < TTL_MS) return cached.data as GitHubRepo[]
    }

    if (!process.env.GITHUB_TOKEN) {
      if (cached) return cached.data as GitHubRepo[]
      return []
    }
    ...
    await supabase.from('github_cache').upsert(
      { key: REPOS_CACHE_KEY, data: repos, updated_at: new Date().toISOString() },
      { onConflict: 'key' }
    )

    return repos
  } catch (err) {
    console.error('[GitHub repos cache] error:', err)
    return []
  }
}

export async function invalidateGitHubReposCache(): Promise<void> {
  try {
    const supabase = createAdminClient()
    await supabase.from('github_cache').delete().eq('key', REPOS_CACHE_KEY)
  } catch {
    // ignore
  }
}
